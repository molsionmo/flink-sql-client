...
package com.github.mxb.flink.sql.executor.local;
 ...
 */
public class LocalCustomCommandLine<T> implements CustomCommandLine {
	private final ClusterClient<T> clusterClient;

	public LocalCustomCommandLine(ClusterClient<T> clusterClient) {
	...
	@Override
	public String getId() {
		return LocalCustomCommandLine.class.getSimpleName();
	}
	...
	public ClusterDescriptor<T> createClusterDescriptor(CommandLine commandLine) {
		return new LocalClusterDescriptor<>(clusterClient);
	}
